...
export const __getCommentsThunk = createAsyncThunk('GET_COMMENTS', async (_, thunkAPI) => {
  try {
...
export const __getCommnetsByTodoId = createAsyncThunk('GET_COMMENT_BY_TODO_ID', async (arg, thunkAPI) => {
  try {
...
export const __updateComment = createAsyncThunk('UPDATE_COMMENT', async (arg, thunkAPI) => {
  try {
...
export const __addComment = createAsyncThunk('ADD_COMMENT', async (arg, thunkAPI) => {
  try {
...
export const commentsSlice = createSlice({
  ...
  extraReducers: {
    ...
    [__deleteComment.fulfilled]: (state, action) => {
      state.commentsByTodoId.isLoading = false
      const target = state.commentsByTodoId.data.findIndex((comment) => comment.id === action.payload)
    ...
    [__updateComment.fulfilled]: (state, action) => {
      const target = state.commentsByTodoId.data.findIndex((comment) => comment.id === action.payload.id)
